...
import { bulkUploadImages, getPatients, createPatient, generatePresignedUrls, confirmBulkUpload } from '../api'
...
function BulkUpload() {
  ...
  // Direct folder upload progress
  const [uploadProgress, setUploadProgress] = useState({}) // objectName -> percent
  ...
  // Folder selection handler (webkitdirectory)
  const handleFolderSelect = (e) => {
    ...
    filesList.forEach(file => {
      ...
      } else if (!file.name.toLowerCase().includes('annotation') && !file.name.toLowerCase().includes('.json')) {
    ...
    // Detect annotation file in folder
    const ann = filesList.find(f => f.name.toLowerCase() === 'annotations.json' || (f.name.toLowerCase().includes('annotation') && f.name.toLowerCase().endsWith('.json')))
    if (ann) {
      setAnnotationFile(ann)
      toast.success('✓ Đã tìm thấy file annotations.json trong folder')
      // try to parse preview
      ann.text().then(text => {
        try {
          const coco = JSON.parse(text)
          const preview = {
            totalImages: coco.images?.length || 0,
            totalAnnotations: coco.annotations?.length || 0,
            categories: coco.categories?.map(c => c.name) || []
          }
          setAnnotationPreview(preview)
        } catch (err) {
          toast.error('Lỗi khi parse file annotation: ' + err.message)
        }
      })
    } else {
      toast.error('⚠️ Không tìm thấy file annotations.json trong folder. Vui lòng đảm bảo folder có file này!', { duration: 5000 })
  ...
  const handleUpload = async () => {
    ...
    if (!annotationFile) {
      toast.error('⚠️ Không tìm thấy file annotations.json trong folder. Vui lòng chọn lại folder có chứa file này!');
      return
    }
    ...
    // Direct-to-MinIO folder upload flow (always)
  ...
  // Upload files directly to MinIO using presigned URLs, then call confirm endpoint
  const handleDirectUpload = async () => {
    try {
      ...
      // Check annotation presence in selected files or separate annotationFile
      const annFile = files.find(f => f.name.toLowerCase() === 'annotations.json' || f.name.toLowerCase().includes('annotation')) || annotationFile

      if (!annFile) {
        toast('⚠️ Không tìm thấy file annotation trong folder. Bạn có thể tiếp tục nhưng sẽ không có annotation', { icon: '⚠️' })
        const cont = window.confirm('Không tìm thấy file annotation trong folder. Bạn có muốn tiếp tục?')
        if (!cont) return
      }

      // Build metadata and confirm patient mappings
      ...
      // Generate a stable prefix for upload object names
      const uploadPrefix = `bulk_uploads/${Date.now()}_${Math.random().toString(36).slice(2)}`

      // Prepare files list for presigned URL generation
      const filesForPresign = files.map(f => {
        const relative = f.webkitRelativePath || f.name
        const objectName = `${uploadPrefix}/${relative}`
        return { objectName, contentType: f.type || 'application/octet-stream', originalName: f.name }
      })
      ...
      const presignResp = await generatePresignedUrls({ files: filesForPresign })
      const presigned = presignResp.data.data || []

      // Mapping from objectName -> url
      const urlMap = {}
      presigned.forEach(p => {
        urlMap[p.objectName] = p.url
      })
      ...
      const urlFilePairs = filesForPresign.map(f => ({ ...f, url: urlMap[f.objectName] }))

      if (urlFilePairs.some(p => !p.url)) {
        throw new Error('Missing presigned URLs for some files')
      }
      ...
      const workers = new Array(concurrency).fill(null).map(async () => {
        while (true) {
          const i = index++
          if (i >= urlFilePairs.length) break
          const item = urlFilePairs[i]
          const fileObj = files.find(f => f.name === item.originalName)
          if (!fileObj) continue

          try {
            await axios.put(item.url, fileObj, {
              headers: { 'Content-Type': fileObj.type || 'application/octet-stream' },
              onUploadProgress: (ev) => {
                const pct = Math.round((ev.loaded / ev.total) * 100)
                setUploadProgress(prev => ({ ...prev, [item.objectName]: pct }))

                // Recompute overall progress
                const sumPercent = Object.values({ ...prev, [item.objectName]: pct }).reduce((a, b) => a + b, 0)
                const overall = Math.round(sumPercent / files.length)
                setOverallProgress(overall)
              }
            })

            uploadedBytes += fileObj.size
            // update overall progress as fallback
            const overallPercent = Math.round((uploadedBytes / totalBytes) * 100)
            setOverallProgress(overallPercent)

          } catch (err) {
            console.error('Upload error for', item.objectName, err.message)
            throw err
          }
        }
      })
      ...
      // After all uploads, call confirm endpoint
      const filesForConfirm = filesForPresign.map(f => ({ objectName: f.objectName, originalName: f.originalName, contentType: f.contentType }))
      const annotationObjectName = annFile ? `${uploadPrefix}/${annFile.webkitRelativePath || annFile.name}` : null

      const confirmResp = await confirmBulkUpload({ metadata: enrichedMetadata, files: filesForConfirm, annotationObjectName })

      const resultData = confirmResp.data.data
      ...
      toast.error('Upload thất bại: ' + (error.response?.data?.error || error.message))
    } finally {
      setUploading(false)
    }
  }
  ...
  return (
    <div className="bulk-upload-container">
      <div className="card">
        ...
        {/* Folder / Drag and Drop Mode Toggle */}
        <div style={{ marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <input type="checkbox" checked={directUploadMode} onChange={(e) => setDirectUploadMode(e.target.checked)} />
            <span style={{ fontSize: '13px' }}>Upload folder trực tiếp (khuyến nghị cho folder lớn ~2GB)</span>
          </label>

          {directUploadMode && (
            <>
              <button className="button" onClick={() => folderInputRef.current?.click()}>Chọn folder</button>
              <input
                ref={folderInputRef}
                type="file"
