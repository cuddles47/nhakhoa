...
class Visit {
    ...
    static async create(visitData) {
        const { patient_id, case_id, visit_date, status, notes, created_by } = visitData;
        const query = `
            INSERT INTO visits (patient_id, case_id, visit_date, status, notes, created_by)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING *
        `;
        const result = await db.query(query, [patient_id, case_id, visit_date, status, notes, created_by]);
        return result.rows[0];
    }

    static async update(id, visitData) {
        const { status, notes } = visitData;
        const query = `
            UPDATE visits 
